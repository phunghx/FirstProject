...
%PYTHON = '/data3/FreeWork/caffeVDSR/VDSR/AWS/';
...
for ix=2:4
    up_scale  = ix;
    PYTHON = sprintf('/data3/FreeWork/caffeVDSR/VDSR/New_v2/Results_%dx/',ix);
    fprintf('---%dx---\n',ix);
    for i_folder=1:size(folders,2)
        ...
        for i_file=3:size(filenames,1),
            ...
            pythonFile = [pythonFile{1} '__VD.mat'];


            %img3 = double(imread([PYTHON folders{i_folder} '/' pythonFile]))/65535.0 * 255.0;            
            load([PYTHON folders{i_folder} '/' pythonFile]);
